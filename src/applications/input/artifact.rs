...
pub struct PyArtifact {
    ...
    pub star: i32,
    #[pyo3(get, set)]
    pub sub_stats: Vec<(String, f64)>,
    #[pyo3(get, set)]
    pub main_stat: (String, f64),
    #[pyo3(get, set)]
    pub id: u64,
}
...
impl PyArtifact {
    ...
    pub fn py_new(
        ...
        level: i32,
        star: i32,
        sub_stats: Vec<(String, f64)>,
        main_stat: (String, f64),
        id: u64,
    ) -> PyResult<Self> {
        Ok(Self {
            set_name,
    ...
    pub fn __repr__(&self, py: Python) -> PyResult<String> {
        let set_name = self.set_name.as_str();
        let slot = self.slot.as_str()?;
        let main_stat = self.main_stat.0.as_str()?;
    ...
    pub fn __dict__(&self, py: Python) -> PyResult<PyObject> {
        ...
        dict.set_item("set_name", self.set_name.as_ref())?;
        dict.set_item("slot", self.slot.as_ref())?;
        dict.set_item("level", self.level)?;
        dict.set_item("star", self.star)?;
        let sub_stats_pylist = PyList::new(
            py,
            self.sub_stats.iter().map(|(s, v)| {
                let stat_str = s.to_str().unwrap();
                (stat_str, *v)
            }),
        );
        dict.set_item("sub_stats", &sub_stats_pylist)?;
        let main_stat_tuple = (self.main_stat.0.as_ref(), self.main_stat.1);
...
impl TryInto<MonaArtifact> for PyArtifact {
    ...
    fn try_into(self) -> Result<MonaArtifact, Self::Error> {
        let name: ArtifactSetName = Python::with_gil(|py| {
            let _string = PyString::new(py, &self.set_name);
            depythonize(&_string).map_err(|err| {
                let serialized_data = format!("{:?}", self.set_name);
                anyhow!(
        ...
        let slot: ArtifactSlotName = Python::with_gil(|py| {
            let _string = PyString::new(py, &self.slot);
            depythonize(&_string).map_err(|err| {
        ...
        let main_stat_name: StatName = Python::with_gil(|py| {
            let _string = PyString::new(py, &self.main_stat.0);
            depythonize(&_string).map_err(|err| {
                let serialized_data = format!("{:?}", self.main_stat.0);
                anyhow!(
        ...
        let sub_stats = Python::with_gil(|py| {
            self.sub_stats
                .iter()
                .map(|s| {
                    let sub_stats = s.0.as_ref();
                    let name: Result<StatName, anyhow::Error> = depythonize(s.0.as_ref()).map_err(|err| {
